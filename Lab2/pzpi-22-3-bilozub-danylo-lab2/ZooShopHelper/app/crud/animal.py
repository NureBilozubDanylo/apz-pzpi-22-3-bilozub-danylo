...
# для float32 і float64
def adapt_numpy_float(n):
    return AsIs(float(n))
...
def update_climate_settings(db: Session, shop_id: int):
    ...
    if animals:
        temperatures = [float(a.temperature) for a in animals]
        humidities = [float(a.humidity) for a in animals]
        light_intensities = [float(a.light_intensity) for a in animals]
        ...
        if high_humidity_animals:
            high_humidity_values = [float(a.humidity) for a in high_humidity_animals]
