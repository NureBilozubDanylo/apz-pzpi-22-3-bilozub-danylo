...
@router.delete("/{shop_id}/{supply_id}", response_model=ShopSupplies)
def delete_shop_supply_by_shop_and_supply(shop_id: int, supply_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_admin_user)):
    db_shop_supplies = crud_shop_supplies.get_shop_supplies_by_shop_and_supply(db, shop_id=shop_id, supply_id=supply_id)
    if db_shop_supplies is None:
        raise HTTPException(status_code=404, detail="Shop supply not found")
    
    # Cascade delete related SaleDetails records
    related_sale_details = db.query(SaleDetails).filter(SaleDetails.shop_supplies_id == db_shop_supplies.shop_supplies_id).all()
    for sale_detail in related_sale_details:
        db.delete(sale_detail)
    
    # Delete the ShopSupplies record
